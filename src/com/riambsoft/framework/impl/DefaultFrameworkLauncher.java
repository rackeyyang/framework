...
public class DefaultFrameworkLauncher implements FrameworkLauncher {
	...
	public BundleContext start() throws FrameworkException {
		...
		//MUST be set to false because many TCK bundles set and read system properties.
		System.setProperty("osgi.framework.useSystemProperties", "false");
		
		try {
			URL[] urls = {new URL((String) map.get("osgi.framework"))};

			frameworkClassLoader = new ChildFirstURLClassLoader(urls, getClass().getClassLoader());
			
			Class<?> clazz = frameworkClassLoader.loadClass("org.eclipse.core.runtime.adaptor.EclipseStarter", true);
			...
			BundleContext bundleContext = (BundleContext)runMethod.invoke(null, new Object[] {args, null});
	...
	public void stop() throws FrameworkException {
		...
		try {
			Class<?> clazz = frameworkClassLoader.loadClass("org.eclipse.core.runtime.adaptor.EclipseStarter");
			...
			clazz = getClass().getClassLoader().loadClass("org.apache.commons.logging.LogFactory");
	...
	public void deploy() throws FrameworkException {
		...
		logger.debug("platformDirectory : " + platformDirectory.getAbsolutePath());
		
		copyResource("/WEB-INF/eclipse/configuration/", new File(
				platformDirectory, "configuration"));

		copyResource("/WEB-INF/eclipse/features/", new File(platformDirectory,
				"features"));
		...
		copyResource("/WEB-INF/eclipse/.eclipseproduct", new File(platformDirectory, ".eclipseproduct"));
	}
	...
	protected Map<Object, Object> buildInitialPropertyMap() throws FrameworkException {
		...
		try {
			if (map.get("osgi.install.area") == null) {
				map.put("osgi.install.area", platformDirectory
			...
			if (map.get("osgi.instance.area") == null) {
				...
				map.put("osgi.instance.area", workspaceDirectory
						.toURI().toURL().toExternalForm());
			}
			if (map.get("osgi.framework") == null) {
				String installArea = (String) map
						.get("osgi.install.area");
				...
				map.put("osgi.framework", new File(path).toURI()
	...
	protected void buildFrameworkPropertyMap(Map<Object, Object> map){
		URL location = Thread.currentThread().getContextClassLoader().getResource("framework.properties");
		Properties result = new Properties();
		InputStream in = null;
		try{
			if(location != null){
			...
			logger.warn("读取Riambosoft Framework 配置文件 framework.properties 异常", e);
		} finally{
			if(in != null){
				try {
					in.close();
		...
		String includes =(String)result.get("scanning.package.includes");
		if(includes != null){
			String packages = (String)map.get("org.osgi.framework.system.packages");
			map.put("org.osgi.framework.system.packages", (packages != null && !"".equals(packages.trim()))? packages + ", "+ includes : includes);	
			logger.debug("导入到OSGI框架的JAVA包  ：" + map.get("org.osgi.framework.system.packages"));
		}
		String autostart = (String)result.get("scanning.bundle.autostart");
		if(autostart != null){
			String bundles = (String)map.get("osgi.bundles");
			map.put("osgi.bundles", (bundles != null && !"".equals(bundles.trim())) ? bundles + "," + autostart : autostart);
	...
	protected String[] buildCommandLineArguments() {
		...
		String commandLine = servletContext.getInitParameter(FrameworkConstants.CONFIG_COMMANDLINE);
		if (commandLine != null) {
	...
	private void registerRestartHandler(Class<?> starterClazz)
		...
		try {
			...
			logger.info(starterClazz.getName() + " 不支持设置 shutdown handler, 故不可设置重启");
			return;
		}
	...
	private Runnable createRestartHandler() throws ClassNotFoundException,
		...
		Class<?> frameworkPropertiesClazz = frameworkClassLoader.loadClass("org.eclipse.osgi.framework.internal.core.FrameworkProperties");
		final Method getProperty = frameworkPropertiesClazz.getMethod("getProperty", new Class[] { String.class });
			...
			public void run() {
				try {
					...
					if (Boolean.valueOf(forcedRestart).booleanValue()) {
						DefaultFrameworkLauncher.this.stop();
						DefaultFrameworkLauncher.this.start();
					}
